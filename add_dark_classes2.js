...
const replacements = [
  ...
  { regex: /bg-white\/(\w+)(?!\s*dark:)/g, replace: 'bg-white/$1 dark:bg-gray-950/$1' },
  ...
  { regex: /border-gray-200\/(\w+)(?!\s*dark:)/g, replace: 'border-gray-200/$1 dark:border-gray-800/$1' },
...
function processFile(filePath) {
  ...
  content = content.replace(classNameRegex, (match, quote, classes) => {
    let updatedClassStr = classes;
    
    for (const { regex, replace } of replacements) {
      updatedClassStr = updatedClassStr.replace(regex, replace);
    }
    
    return `className=${quote}${updatedClassStr}${quote}`;
  });
