  ...
  return (
    <div className="relative max-w-4xl mx-auto px-4 py-8 md:py-12 min-h-[85vh] flex flex-col justify-start items-center font-sans">
      ...
      >
        ...
        <div className="relative w-full max-w-100 aspect-square mx-auto bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg p-2 md:p-4 touch-none">
          {status === 'waiting' && (
          ...
          {board.length > 0 && (
            <div 
              className={`w-full h-full grid ${currentLevel.size > 5 ? 'gap-0.5 md:gap-1' : 'gap-1 md:gap-2'}`}
              style={{
              ...
              {board.map((cell, idx) => {
                ...
                return (
                  <div 
                    ...
                    className={`w-full h-full min-h-0 min-w-0 flex items-center justify-center bg-white rounded-md overflow-hidden ${!cell.isFixed ? 'cursor-pointer hover:bg-gray-50 active:bg-gray-100 shadow-sm border border-gray-100' : ''}`}
                  >
                    <motion.div
