...
function GameCard({ game, isAvailable }) {
  return (
    <div className={`relative h-full flex flex-col bg-white dark:bg-[#111111] rounded-xl md:rounded-2xl border ${isAvailable ? 'border-gray-200 dark:border-gray-800 hover:border-[var(--amethyst)]/50 dark:hover:border-[var(--amethyst)]/50 hover:shadow-[0_12px_30px_-10px_rgba(0,0,0,0.1)] dark:hover:shadow-[0_12px_30px_-10px_rgba(138,43,226,0.15)] hover:-translate-y-1.5' : 'border-gray-200 dark:border-gray-800 opacity-60'} transition-all duration-500 ease-out overflow-hidden group p-4 md:p-8 z-10`}>
      
      {/* Subtle hover gradient blob overlay */}
      {isAvailable && (
        <div className="absolute -z-10 -inset-px rounded-xl md:rounded-2xl bg-gradient-to-br from-[var(--amethyst)]/0 via-transparent to-[var(--amethyst)]/0 group-hover:from-[var(--amethyst)]/5 dark:group-hover:from-[var(--amethyst)]/10 group-hover:to-transparent transition-all duration-500 opacity-0 group-hover:opacity-100"></div>
      )}

      {/* Top right status badge */}
      <div className="absolute top-3 right-3 md:top-5 md:right-5 flex items-center justify-center">
        <span className={`flex items-center gap-1.5 text-[8px] sm:text-xs font-bold uppercase tracking-widest px-2 py-1 md:px-3 md:py-1.5 rounded-md md:rounded-full backdrop-blur-sm border ${isAvailable ? 'bg-[var(--honeydew)]/80 dark:bg-[#1a1a1a]/80 border-[var(--amethyst)]/15 dark:border-gray-800 text-[var(--chrysler-blue)] dark:text-gray-300' : 'bg-gray-50/80 dark:bg-[#1a1a1a]/80 border-gray-200 dark:border-gray-800 text-gray-400 dark:text-gray-500'}`}>
          <span className={`w-1.5 h-1.5 rounded-full ${isAvailable ? 'bg-[var(--amethyst)] group-hover:animate-pulse' : 'bg-gray-400 dark:bg-gray-600'}`}></span>
      ...
      <div className="flex-grow z-10">
        <h3 className={`text-base md:text-2xl font-bold mb-1 md:mb-3 ${isAvailable ? 'text-gray-900 dark:text-gray-100 group-hover:text-[var(--amethyst)] dark:group-hover:text-[var(--amethyst)] transition-colors duration-300' : 'text-gray-500 dark:text-gray-500'} line-clamp-1`}>
          {game.title}
        </h3>
      ...
      {isAvailable && (
        <div className="mt-auto pt-3 md:pt-5 border-t border-gray-100 dark:border-gray-800/80 flex items-center justify-between font-medium z-10 transition-colors duration-300">
          <span className="flex items-center gap-1 sm:gap-2 text-[10px] sm:text-sm md:text-base text-gray-700 dark:text-gray-300 group-hover:text-[var(--amethyst)] dark:group-hover:text-[var(--amethyst)] transition-colors duration-300">
